...
--for making lines
function newParticleLayer()
	table.insert(allParticles, {})
	particles = allParticles[#allParticles]
end
...
function drawParticles()
	for i = 1, #allParticles do
		local layer = allParticles[i]
		for j = 1, #layer do
			local p = layer[j]
			love.graphics.setColor(p.r, p.g, p.b, p.a)
			if p.image then 
				--draw image
				love.graphics.draw(p.image, p.x, p.y, 0, p.size, p.size)
			else
...
function addParticle(x, y, meta, extraParams)
	...
	--did you think this was going to be hard or something?
	if meta.image then
		p.image = meta.image
	end
